...
MAX_WAIT_TIME = 12 * 60 * 60 * 20 # assume maximal wait time is 12 hours.
MAX_RUN_TIME = 12 * 60 * 60 * 20 # assume maximal runtime is 12 hours
...
class HPCEnv(gym.Env):
    ...
    def reset_for_test(self, num,start):
        ...
        ###self.current_timestamp = self.loads[self.start].submit_time
        ###self.job_queue.append(self.loads[self.start])
        random.seed(self.start)
        random_submits = random.sample(range(1000), num)
        sorted_submits = sorted(random_submits)
        for i in range(num):
            self.loads[self.start + i].submit_time = -sorted_submits[i]
            self.job_queue.append(self.loads[self.start+i])
        #self.loads[self.start].submit_time = 0
        self.current_timestamp = self.loads[self.start].submit_time
        self.next_arriving_job_idx = self.start + 1
    ...
    #@profile
    ###
    def schedule_curr_sequence_reset_cust(self, cust_order):
        # schedule the sequence of jobs using heuristic algorithm.
        ori_queue_len = len(self.job_queue)
        scheduled_logs = {}
        # f = False
        ...
        self.job_queue = [self.job_queue[i] for i in cust_order]
        while True:
        ...
        return sum(scheduled_logs.values())
    ###
    def schedule_curr_sequence_reset_optimal(self):
        import itertools
        # schedule the sequence of jobs using heuristic algorithm.
        optimal_score = 999999
        for permu in itertools.permutations(self.job_queue):
            ori_queue_len = len(self.job_queue)
            scheduled_logs = {}
            self.job_queue = list(permu)
            while True:
                ###self.job_queue.sort(key=lambda j: score_fn(j))
                job_for_scheduling = self.job_queue[0]
                # if selected job needs more resources, skip scheduling and try again after adding new jobs or releasing some resources
            ...
            self.last_job_in_batch = self.start + self.num_job_in_batch
            self.next_arriving_job_idx = self.start + 1
            curr_score = sum(scheduled_logs.values())
    ...
    def schedule_curr_sequence_reset_op(self, score_fn, given_order):
        hill_time=0.1
        bb_time=0.1
        #print([self.job_queue[0]])
        #print([self.job_queue[0].job_id])
        #print([self.job_queue[0].submit_time])
        #print([self.job_queue[0].run_time])
        #print([self.job_queue[0].number_of_allocated_processors])
        
        # ---------- 初始化顺序 ----------
        if given_order == []:
            indexed_list = list(enumerate(self.job_queue))
            indexed_list.sort(key=lambda x: score_fn(x[1]))
            init_order = [index for index, value in indexed_list]
        else:
            init_order = [i for i in given_order]
        ...
        def sched_sim(order):
            ...
            self.job_queue = [self.job_queue[i2] for i2 in order]
        ...
        best_score = sched_sim(best_order)

        start = time.time()
        while time.time() - start < hill_time:
            cand = best_order[:]
        ...
        class Node:
            def __init__(self, expl, unexpl, cost, lb):
                self.expl = expl
                self.unexpl = unexpl
                self.cost = cost      # full order cost (upper bound)
                self.lb = lb          # prefix cost (lower bound)

            def __lt__(self, other):
                return (len(self.expl), -self.cost) < (len(other.expl), -other.cost)


        c_star = best_score
        best_node = Node([], best_order[:], best_score, 0)

        Q = []
        heapq.heappush(Q, best_node)

        start = time.time()
        while Q and time.time() - start < bb_time:
            node = heapq.heappop(Q)

            # prune
            if not node.unexpl or node.lb > c_star:
                continue
            ...
            for j in node.unexpl:
                if time.time() - start >= bb_time:
                    break

                child_expl = node.expl + [j]
                child_unexpl = [x for x in node.unexpl if x != j]

                # child_unexpl_atom = [x for x in child_unexpl if self.job_queue[x]]
                child_lb = sched_sim(child_expl)
                if child_lb > c_star:
                    continue

                full_order = child_expl + child_unexpl
                child_cost = sched_sim(full_order)

                heapq.heappush(
                    Q,
                    Node(child_expl, child_unexpl, child_cost, child_lb)
                )

        return c_star, best_node.expl + best_node.unexpl
    ...
    def job_score(self, job_for_scheduling):
        ...
        if self.job_score_type == 0:
            ...
            _tmp = max(1.0, (float(job_for_scheduling.scheduled_time - job_for_scheduling.submit_time + job_for_scheduling.run_time)
                            /
                            max(job_for_scheduling.run_time, 300)))
        elif self.job_score_type == 1:
            #wait time
    ...
    def step_for_test(self, a):
        ...
        if not done:
            obs = self.build_observation()
            return [obs, 0, False, job_for_scheduling]#None
        else:
            self.post_process_score(self.scheduled_rl)
            rl_total = sum(self.scheduled_rl.values())
            ### print(list(self.scheduled_rl.values()))
            return [None, rl_total, True, job_for_scheduling]#list(self.scheduled_rl.values())
