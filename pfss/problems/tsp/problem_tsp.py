...
class TSPDataset(Dataset):
    ...
    def __init__(self, filename=None, min_size=20, machine=5, max_size=50, batch_size=128,
        ...
        if filename is not None:
            ...
            self.tour_nodes = []
            for i in tqdm(range(num_samples)):
                self.nodes_coords.append([[0.100,0.192],[0.290,0.256],[0.300,0.192],[0.400,0.64]])
                self.tour_nodes.append(np.array([3,0,2,1]))
            self.nodes_coords = np.array(self.nodes_coords)
            '''
            import json
            
            #self.nodes_coords = []
            #self.tour_nodes = []
            with open("nodes_coords.json", 'r') as filen:
                self.nodes_coords = json.load(filen)
            with open("tour_nodes.json", 'r') as filet:
                self.tour_nodes = json.load(filet)
        ...
        ###
        #print(type(self.nodes_coords))
        #print(type(self.tour_nodes))
        #print(self.nodes_coords)
        #print(self.tour_nodes)
        #exit(0)
