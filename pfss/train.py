...
def rollout(model, dataset, opts):
    ...
    def eval_model_bat(bat):
        import json
        with torch.no_grad():
            cost, _, pi = model(nodes=move_to(bat['nodes'], opts.device), graph=move_to(bat['graph'], opts.device), return_pi=True)
            print(pi)
        with open('../tour_nodes_cust.json', 'w') as filet:
            json.dump(pi.tolist(), filet)
...
def rollout_groundtruth(problem, dataset, opts):
    for bat in DataLoader(dataset, batch_size=opts.batch_size, shuffle=False, num_workers=opts.num_workers):
        print(bat['tour_nodes'])
    return torch.cat([
...
def train_epoch(model, optimizer, baseline, lr_scheduler, epoch, val_datasets, problem, tb_logger, opts):
    ...
    train_dataset = baseline.wrap_dataset(
        problem.make_dataset(filename='sl', 
    ...
    # if avg_opt_gap < best_gap:
    #     best_gap = avg_opt_gap
    #     print('Saving model and state...')
    #     torch.save(
    #         {
    #             'model': get_inner_model(model).state_dict(),
...
def train_epoch_sl(model, optimizer, lr_scheduler, epoch, train_dataset, val_datasets, problem, tb_logger, opts, f):
    ...
    # if (opts.checkpoint_epochs != 0 and epoch % opts.checkpoint_epochs == 0) or epoch == opts.n_epochs - 1:
    if avg_opt_gap < best_gap and epoch % 10 == 9:
        best_gap = avg_opt_gap
        print('Saving model and state...')
        torch.save(
            {
